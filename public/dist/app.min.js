...
var CorpusModule=require("./modules/corpus"),app=angular.module("context-app",["ngRoute","ngAnimate","mgcrea.ngStrap","CorpusModule","vs-repeat","pasvaz.bindonce","infinite-scroll"]);app.filter("pagination",function(){return function(input,start){return input?input.slice(start):void 0}}),require("./routes")(app),require("./services")(app),require("./controllers")(app),require("./directives")(app),require("./filters")(app);
},{"./controllers":2,"./directives":15,"./filters":16,"./modules/corpus":19,"./routes":20,"./services":21}],2:[function(require,module,exports){
...
var types,entities,articles,totalDisplayed=15;module.exports=function($scope,$state,$sce){$scope.typeFilter=void 0,$scope.entityFilter=void 0,$scope.articleFilter=void 0,oboe({url:"/api/corpus/"+$scope.currentCorpus._id+"/facets",cached:!0}).done(function(data){types=data.types,entities=data.entities,articles=data.articles;for(var i=0;i<articles.length;i++)articles[i].sourceSafe=$sce.trustAsHtml(articles[i].source);$scope.data=[{name:"Articles",values:articles},{name:"Types",values:types},{name:"Entities",values:entities}],$scope.curPage=0,$scope.pageSize=100,$scope.$watch("curPage",function(oldValue,newValue){oldValue!==newValue&&(window.scrollTo(0,50),$scope.totalDisplayed=totalDisplayed)}),$scope.numberOfPages=function(filtered){return null!==filtered?Math.ceil(filtered?filtered/$scope.pageSize:$scope.data[0].values.length/$scope.pageSize):void 0},$state.reload()}),$scope.totalDisplayed=totalDisplayed,$scope.loadMore=function(){$scope.totalDisplayed<$scope.pageSize&&($scope.totalDisplayed+=5)},$scope.search=function(){$http.get("/api/corpus/"+$scope.currentCorpus._id+"/search/"+$scope.query).success(function(data){for(var i=0;i<data.length;i++)data[i].sourceSafe=$sce.trustAsHtml(data[i].source);$scope.searchResult=data})},$scope.setFilter=function(item){switch($scope.totalDisplayed=totalDisplayed,$scope.curPage=0,item.type){case"article":$scope.articleFilter===item.id?($scope.articleFilter=void 0,$scope.aF=void 0):($scope.articleFilter=item.id,$scope.aF=item);break;case"entity":$scope.entityFilter===item.id?($scope.entityFilter=void 0,$scope.eF=void 0):($scope.entityFilter=item.id,$scope.eF=item);break;case"type":$scope.typeFilter===item.id?($scope.typeFilter=void 0,$scope.tF=void 0):($scope.typeFilter=item.id,$scope.tF=item)}},$scope.itemActive=function(item){switch(item.type){case"article":return $scope.articleFilter===item.id;case"entity":return $scope.entityFilter===item.id;case"type":return $scope.typeFilter===item.id}},$scope.facetFilter=function(item){var filterArticle=!1,filterEntity=!1,filterType=!1;switch(item.type){case"article":return filterEntity=void 0!==$scope.entityFilter?-1!==item.entities.indexOf($scope.entityFilter):!0,filterType=void 0!==$scope.typeFilter?-1!==item.types.indexOf($scope.typeFilter):!0,filterEntity&&filterType;case"entity":return filterArticle=void 0!==$scope.articleFilter?-1!==item.articles.indexOf($scope.articleFilter):!0,filterType=void 0!==$scope.typeFilter?-1!==item.types.indexOf($scope.typeFilter):!0,filterArticle&&filterType;case"type":return filterArticle=void 0!==$scope.articleFilter?-1!==item.articles.indexOf($scope.articleFilter):!0,filterEntity=void 0!==$scope.entityFilter?-1!==item.entities.indexOf($scope.entityFilter):!0,filterArticle&&filterEntity}}};
},{}],5:[function(require,module,exports){
var extentions=require("../../data/corpusextentions"),defaultPort="8081",currentCorpus={uri:function(){var it=this.input_type;return"twitter"===it?"http://twitter.com/"+this.input:"direct"===it?"-":this.input}},initProgressWebsocket=function($scope){var location=document.location.hostname+":"+defaultPort+document.location.pathname.replace("/overview",""),socket=new WebSocket("ws://"+location);socket.onerror=function(){},socket.onopen=function(){$scope.$apply(function(){$scope.progress.show=!0})},socket.onmessage=function(event){var data=JSON.parse(event.data);$scope.$apply(function(){$scope.progress.type=data.type,$scope.progress.progress=data.progress})},socket.onclose=function(){$scope.$apply(function(){$scope.progress.show=!1})}};module.exports=function($scope,$location,$state,corpus){currentCorpus=_.extend(currentCorpus,corpus.data),$scope.currentCorpus=currentCorpus,$scope.Math=Math,$scope.progress={show:!1,progress:0,type:""},$scope.extentions=extentions,$scope.$state=$state,$scope.$on("$viewContentLoaded",function(){currentCorpus.processed||initProgressWebsocket($scope);var len=$location.path().split("/").length;3===len&&$state.go("corpus."+extentions[0].name)})};
...
},{"../../modules/RelationsProcesser":18}],8:[function(require,module,exports){
...
},{"C:\\Users\\Berker\\WebstormProjects\\context-repo\\public\\js\\controllers\\corpus\\overview.js":6}],11:[function(require,module,exports){
module.exports=require(6)
},{"C:\\Users\\Berker\\WebstormProjects\\context-repo\\public\\js\\controllers\\corpus\\overview.js":6}],12:[function(require,module,exports){
var collection=[];collection.push({name:"feed",description:"RSS/RDF/ATOM Feed",inputType:"text",inputDescription:"URL of the RSS/RDF/ATOM feed",inputPlaceholder:"URL of your feed",haveItems:!0,itemsMin:50,itemsMax:600,itemsDefault:50}),collection.push({name:"wordpress",description:"WordPress Blog",inputType:"text",inputDescription:"Weblog URL",inputPlaceholder:"URL of the blog",haveItems:!0,itemsMin:50,itemsMax:1e3,itemsDefault:500}),collection.push({name:"blogger",description:"Blogger Blog",inputType:"text",inputDescription:"Weblog URL",inputPlaceholder:"URL of the blog",haveItems:!0,itemsMin:50,itemsMax:400,itemsDefault:400}),collection.push({name:"twitter",description:"Public Twitter account",inputType:"text",inputDescription:"Twitter username",inputPlaceholder:"Username",haveItems:!0,itemsMin:200,itemsMax:5e3,itemsDefault:1e3}),collection.push({name:"slidewiki",description:"SlideWiki Deck",inputType:"text",inputDescription:"Deck ID",inputPlaceholder:"Deck ID",haveItems:!0,itemsMin:50,itemsMax:1e3,itemsDefault:400}),collection.push({name:"webpage",description:"Web page",inputType:"text",inputDescription:"WebPage URL",inputPlaceholder:"URL",haveItems:!1}),collection.push({name:"directinput",description:"Direct input",inputType:"textarea",inputDescription:"Your text/html",haveItems:!1}),collection.push({name:"doc",description:"Upload a document(s)",inputType:"file",inputDescription:"Document",haveItems:!1}),module.exports=collection;
...
},{"../controllers/corpus/main":5,"../data/corpusextentions":13}],20:[function(require,module,exports){
...
module.exports=require(6)
},{"C:\\Users\\Berker\\WebstormProjects\\context-repo\\public\\js\\controllers\\corpus\\overview.js":6}]},{},[1]);
